<?php 
    ...
    class login extends utilidades{
        ...
        public function autenticar($params = null) {
            ...
            $usuario    = $this->cleanQuery($params["usuario"] ?? '');
            $contrasena = $this->cleanQuery($params["contrasena"] ?? '');

            // 1) Verificar credenciales
            $query = "SELECT mu.id_usuario, mu.usuario,mu.perfil_id,mu.password,mu.celular,mu.nombre,cp.perfil
                    FROM master_usuarios mu
                    INNER JOIN catalogo_perfiles cp ON mu.perfil_id = cp.id_perfil
                    WHERE mu.usuario = '$usuario' AND mu.status = 1 LIMIT 1";
            ...
            $permisos = $this->setPermisosPerfil([
                "perfil_id" => $usuario['perfil_id']
            ]);
            session_start();
            $_SESSION['intermedica'] = true;
            $_SESSION['usuario'] = $usuario;
            $_SESSION['permisos'] = $permisos['permisos'];
            $menu = $this->generarMenu($permisos['permisos']);
            $_SESSION['menu'] = $menu[0]["estructura"];
            $mensaje = "Inicio de sesión exitoso, serás redireccionado al sistema.";
        ...
        public function ResetPass($params = null) {
            ...
            $nuevo = false;
            $celular = 0;
            $usuario = isset($params["usuario"]) ? $this->cleanQuery($params["usuario"]) : "";
            $codigo_recuperacion = isset($params["codigo"]) ? $this->cleanQuery($params["codigo"]) : "";
            ...
            $qry_usuario = "SELECT * FROM master_usuarios WHERE usuario ='".$usuario."' AND status = 1";
            ...
            if ($res && $res->num_rows > 0) {
                ...
                switch ($paso) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        $response = $this->guardarContrasena([
                            'celular'=>$celular,
                            'contrasena'=>$contrasena,
                            'contrasena_r'=>$contrasena_r,
                            'nuevo'=>$nuevo,
                        ]);
                        if($response[0]!= "OK"){
                            $codigo = $response[0];
                ...
                $codigo = "ERR";
                $mensaje = "No se encontro al usuario, intenta de nuevo.";
            }
